...
// A data-video value is treated as a self-hosted file (native <video> tag,
// no YouTube branding) if it points to a local video file extension.
// Otherwise it's treated as a YouTube link and embedded via iframe.
...
function buildModalGallery(slides, videoUrl) {
  ...
  if (videoUrl && videoUrl.trim() !== '') {
    if (isLocalVideoFile(videoUrl)) {
      // Self-hosted video file — plays inline with native browser controls
      const videoWrap = document.createElement('div');
      videoWrap.className = 'media-video-wrap media-video-native';
      const video = document.createElement('video');
      video.src = videoUrl.trim();
      video.controls = true;
      video.playsInline = true;
      video.preload = 'metadata';
      // Use the project's first image as a poster frame, if available
      if (slides.length) video.poster = slides[0];
      videoWrap.appendChild(video);
      modalMediaFeed.appendChild(videoWrap);
    } else {
      // YouTube link — embedded via iframe
      const videoWrap = document.createElement('div');
...
document.querySelectorAll('.project-card').forEach((card) => {
  card.addEventListener('click', () => {
    ...
    // Build the tools/tags row from this card's own data-tags (comma-separated)
    modalTags.innerHTML = '';
    const tagsAttr = card.getAttribute('data-tags') || '';
    tagsAttr.split(',').map(t => t.trim()).filter(Boolean).forEach(tagText => {
...
function closeModal() {
  ...
  // Stop any playing YouTube video by clearing its iframe src
  const iframe = modalMediaFeed.querySelector('iframe');
  if (iframe) iframe.src = '';
  // Pause any playing self-hosted video
  const video = modalMediaFeed.querySelector('video');
